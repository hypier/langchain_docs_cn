...
:::info
前往 [Integrations](/docs/integrations/vectorstores/) 获取与第三方向量存储的内置集成文档。
:::

存储和搜索非结构化数据最常见的方法之一是将其嵌入并存储生成的嵌入向量，然后在查询时嵌入非结构化查询并检索与嵌入查询“最相似”的嵌入向量。向量存储为您处理嵌入数据的存储和向量搜索。

## 开始

本指南展示了与向量存储相关的基本功能。使用向量存储的一个关键部分是创建要放入其中的向量，这通常是通过嵌入生成的。因此，建议您在深入了解之前，先熟悉一下[文本嵌入模型接口](/docs/how_to/embed_text)。
...
有许多优秀的向量存储选项，以下是一些免费的、开源的，并且完全在您的本地机器上运行的选项。查看所有集成以获取许多优秀的托管服务。

<Tabs>
  <TabItem value="chroma" label="Chroma" default>

本演示使用 `chroma` 向量数据库，该数据库作为库在您的本地机器上运行。
...
本演示使用 `FAISS` 向量数据库，该数据库利用了 Facebook AI 相似性搜索 (FAISS) 库。
...
所有的向量存储都暴露了一个 `similarity_search` 方法。这个方法将接收传入的文档，创建它们的嵌入，然后找到所有具有最相似嵌入的文档。
...
还可以使用 `similarity_search_by_vector` 对与给定嵌入向量相似的文档进行搜索，该方法接受嵌入向量作为参数，而不是字符串。
...
向量存储通常作为一个独立的服务运行，需要进行一些IO操作，因此它们可能会被异步调用。这带来了性能优势，因为你不会浪费时间等待外部服务的响应。如果你使用的是异步框架，例如[FastAPI](https://fastapi.tiangolo.com/)，这可能也很重要。

LangChain支持向量存储的异步操作。所有方法都可以使用其异步对应方法调用，前缀为`a`，表示`async`。
