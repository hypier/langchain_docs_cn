...
>[Vearch](https://vearch.readthedocs.io) 是深度学习和人工智能应用的向量搜索基础设施。

## 设置

请按照 [说明](https://vearch.readthedocs.io/en/latest/quick-start-guide.html#) 进行操作。
...
```python
# 添加你的本地知识文件
file_path = "/data/zhx/zhx/langchain-ChatGLM_new/knowledge_base/天龙八部/lingboweibu.txt"  # 你的本地文件路径
...
print("删除 vearch standalone docid", res_d)
query = "你知道vearch是什么吗?"
response, history = model.chat(tokenizer, query, history=[])
print(f"人类: {query}\nChatGLM:{response}\n")
...
print("删除 vearch cluster docid", res_cluster)
...
print("删除后再次查询 docid:", get_delet_doc)
...
```
```output
删除 vearch standalone docid True
人类: 你知道vearch是什么吗?
